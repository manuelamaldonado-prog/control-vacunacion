...
function actualizarListasClientes() {
  ...
  poblarDatalist(
    "dl-cli-renspa",
    Array.from(setRen).sort((a, b) => String(a).localeCompare(String(b), "es"))
  );
  poblarDatalist(
    "dl-cli-nombre",
    Array.from(setNom).sort((a, b) => String(a).localeCompare(String(b), "es"))
  );
  poblarDatalist(
    "dl-cli-documento",
    Array.from(setDoc).sort((a, b) => String(a).localeCompare(String(b), "es"))
  );
  poblarDatalist(
    "dl-cli-localidad",
    Array.from(setLoc).sort((a, b) => String(a).localeCompare(String(b), "es"))
  );
  poblarDatalist(
    "dl-cli-departamento",
    Array.from(setDep).sort((a, b) => String(a).localeCompare(String(b), "es"))
  );
  poblarDatalist(
    "dl-cli-provincia",
    Array.from(setProv).sort((a, b) => String(a).localeCompare(String(b), "es"))
  );
}
...
function renderActasPorVacunador() {
  ...
  lista.sort((a, b) => {
    ...
    if (fa === fb) {
      ...
      return String(aa).localeCompare(String(ab), "es");
    }
    return String(fa).localeCompare(String(fb), "es");
  });
  if (!lista.length) {
    tbody.innerHTML =
...
function actualizarListas() {
  ...
  poblarDatalist(
    "dl-tipo-vacuna",
    Array.from(tipoVacuna).sort((a, b) => String(a).localeCompare(String(b), "es"))
  );
  poblarDatalist(
    "dl-marca",
    Array.from(marca).sort((a, b) => String(a).localeCompare(String(b), "es"))
  );
  poblarDatalist(
    "dl-serie",
    Array.from(serie).sort((a, b) => String(a).localeCompare(String(b), "es"))
  );
  poblarDatalist(
    "dl-fecha-venc",
  ...
  poblarDatalist(
    "dl-comprador",
    Array.from(comprador).sort((a, b) => String(a).localeCompare(String(b), "es"))
  );
  poblarDatalist(
    "dl-receptor",
    Array.from(receptor).sort((a, b) => String(a).localeCompare(String(b), "es"))
  );
  poblarDatalist(
    "dl-entregador",
    Array.from(entregadorSet).sort((a, b) => String(a).localeCompare(String(b), "es"))
  );
  poblarDatalist(
    "dl-recibidor",
    Array.from(recibidorSet).sort((a, b) => String(a).localeCompare(String(b), "es"))
  );
  poblarDatalist(
    "dl-aft-marca",
    Array.from(marcaAftosa).sort((a, b) => String(a).localeCompare(String(b), "es"))
  );
  poblarDatalist(
    "dl-aft-serie",
    Array.from(serieAftosa).sort((a, b) => String(a).localeCompare(String(b), "es"))
  );
  poblarDatalist(
    "dl-aft-venc",
  ...
  poblarDatalist(
    "dl-bru-marca",
    Array.from(marcaBrucelosis).sort((a, b) => String(a).localeCompare(String(b), "es"))
  );
  poblarDatalist(
    "dl-bru-serie",
    Array.from(serieBrucelosis).sort((a, b) => String(a).localeCompare(String(b), "es"))
  );
  poblarDatalist(
    "dl-bru-venc",
...
function renderResumenGlobalVacunas() {
  ...
  const vacunas = Object.keys(agrupado).sort((a, b) => String(a).localeCompare(String(b), "es"));
  const partes = [];
  vacunas.forEach((vac) => {
    ...
    filas.sort((a, b) => String(a.laboratorio).localeCompare(String(b.laboratorio), "es"));
    let totalCompra = 0;
    let totalEntrega = 0;
  ...
  if (formVacunacion) {
    ...
    if (inputRenspaVac) {
      ...
      inputRenspaVac.addEventListener("blur", () => {
        ...
        if (cli) {
          setVal(nom, toTitleCase(cli.nombre || ""));
          setVal(est, toTitleCase(cli.establecimiento || ""));
          setVal(loc, toTitleCase(cli.localidad || ""));
          setVal(dep, toTitleCase(cli.departamento || ""));
          if (selVacunador) {
            const newVac = cli.vacunadorId || "";
            // Only enforce the client's default vaccinator if they actually have one.
            // If they don't, we leave whatever the user might be actively selecting alone.
            if (newVac && selVacunador.value != String(newVac)) {
              selVacunador.value = String(newVac);
            }
          }
        } else {
          setVal(nom, "");
          ...
          alert("ERROR EN BLUR RENSPA: " + err.message + "\nLinea: " + err.lineNumber + "\nStack: " + err.stack);
        }
      });
    }
    ...
    formVacunacion.addEventListener("submit", (e) => {
      ...
      if (!periodo || !tipoVacunacion || !fecha || !renspa || !anio || !acta) {
        alert("Atención: Completá TODOS los campos de la cabecera (Fecha, Año, Período, Tipo, N° Acta y RENSPA).");
        return;
      }
      recalcularTotal();
      const aft = {
      ...
      renderVacunaciones();
        ...
        alert("ERROR EN GUARDAR VACUNACION: " + err.message + "\nLinea: " + err.lineNumber + "\nStack: " + err.stack);
      }
    });
  }
...
function renderInformesGlobales() {
  ...
  arrLotes.sort((a, b) => {
    if (a.vacuna !== b.vacuna) return String(a.vacuna).localeCompare(String(b.vacuna));
  ...
  if (tbodyVac) {
    ...
    if (vMap.size === 0) {
      ...
      vMap.forEach((val, key) => {
        const vacObj = vacunadores.find(x => String(x.id) === String(key));
...
function initialize() {
  ...
  if (vacunadores.length > 0) {
    const defaultVac = vacunadores[0];
    let changed = false;
    vacunaciones.forEach(v => {
      if (!v.vacunadorId) { v.vacunadorId = defaultVac.id; v.vacunadorNombre = defaultVac.nombre; changed = true; }
    });
    if (changed) guardarVacunaciones();

    let changedEnt = false;
    actasEntrega.forEach(a => {
      if (!a.vacunadorId) { a.vacunadorId = defaultVac.id; changedEnt = true; }
    });
    if (changedEnt) guardarActasEntrega();

    let changedRec = false;
    actasRecepcion.forEach(a => {
      if (!a.vacunadorId) { a.vacunadorId = defaultVac.id; changedRec = true; }
    });
    if (changedRec) guardarActasRecepcion();
  }
